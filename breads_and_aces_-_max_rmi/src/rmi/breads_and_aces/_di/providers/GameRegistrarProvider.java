...
import breads_and_aces.registration.init.servable.registrar.registrars.GameRegistrarInit;
import breads_and_aces.registration.init.servable.registrar.registrars.GameRegistrarStarted;
...
public class GameRegistrarProvider implements Provider<GameRegistrar> {
	...
	public void changeRegistrar() {
		...
		printer.println(registeredPlayer.stream().map(Player::getId).collect(Collectors.joining(", "))
		);
	}
